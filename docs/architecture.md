...
| 模块 | 路径 | 职责 |
|------|------|------|
| ModelAdapter | src/tennis_train/adapter.py | 统一训练/推理/量化接口，自动选择 yolo26 或 ultralytics 后端 |
| 训练器 | src/tennis_train/train.py | CLI 训练入口，支持 epochs/batch/device 等参数 |
| 量化器 | src/tennis_train/quantize.py | FP32 ONNX 导出 + 动态/静态 INT8 量化 |
